...
var _dexieEncrypted = require('@aguycalled/dexie-encrypted');
...
class Db extends events.EventEmitter {
  ...
  async Open(filename, secret, indexedDB, IDBKeyRange) {
    ...
    try {
      ...
      this.dbTx = new _dexie.default('___tx___' + filename, {
        indexedDB: indexedDB || window.indexedDB,
        IDBKeyRange: IDBKeyRange || window.IDBKeyRange,
      });
      this.open = true;
      let self = this;
      (0, _dexieEncrypted.applyEncryptionMiddleware)(
        this.db,
        key,
        {
          keys: _dexieEncrypted.NON_INDEXED_FIELDS,
          walletTxs: _dexieEncrypted.NON_INDEXED_FIELDS,
          outPoints: _dexieEncrypted.NON_INDEXED_FIELDS,
          scriptHistories: _dexieEncrypted.NON_INDEXED_FIELDS,
          settings: _dexieEncrypted.NON_INDEXED_FIELDS,
          encryptedSettings: _dexieEncrypted.NON_INDEXED_FIELDS,
          statuses: _dexieEncrypted.NON_INDEXED_FIELDS,
          stakingAddresses: _dexieEncrypted.NON_INDEXED_FIELDS,
          labels: _dexieEncrypted.NON_INDEXED_FIELDS,
          names: _dexieEncrypted.NON_INDEXED_FIELDS,
          tokens: _dexieEncrypted.NON_INDEXED_FIELDS,
          nfts: _dexieEncrypted.NON_INDEXED_FIELDS,
        },
        async (db) => {
      ...
      this.dbTx.on('versionchange', function (event) {
        self.dbTx.close();
      });
  ...
  static SetBackend(indexedDB, IDBKeyRange) {
    _dexie.default.dependencies.indexedDB = indexedDB;
    _dexie.default.dependencies.IDBKeyRange = IDBKeyRange;
  }
  ...
  static async ListWallets() {
    return (await _dexie.default.getDatabaseNames()).filter(
  ...
  static async RemoveWallet(filename) {
    try {
      await _dexie.default.delete(filename);
      return true;
    } catch (e) {
      console.log(e);
      return false;
  ...
  async AddKey(hashId, value, type, address, used, change, path, password) {
    ...
    if (type != _address_types.default.XNAV) {
      password = this.HashPassword(password);
      value = this.Encrypt(value, password);
    }
  ...
  async GetKey(key, password) {
    ...
    if (dbFind.type != _address_types.default.XNAV) {
      try {
        const ciphertextBytes = Buffer.from(ret, 'base64');
        const iv = ciphertextBytes.slice(0, 16);
        const data = ciphertextBytes.slice(16);
  ...
  async GetNavAddresses() {
    ...
    return await this.db.keys
      .where({
        type: _address_types.default.NAV,
      })
      .toArray()
      .catch((e) => {
  ...
  async GetXNavReceivingAddresses(all) {
    ...
    let ret = await this.db.keys
      .where({
        type: _address_types.default.XNAV,
      })
      .toArray()
      .catch((e) => {
  ...
  async GetUtxos() {
    ...
    if (!this.db) return [];
    let ret = await this.db.outPoints.where({
      spentIn: '',
    });
    if (forBalance)
      ret.or({
  ...
  async AddUtxo(outPoint, out, spentIn, amount, label, type) {
    ...
    let votingPk =
      arguments.length > 8 && arguments[8] !== undefined ? arguments[8] : '';
    let hashId =
...
exports.default = Db;
const ListWallets = Db.ListWallets;
exports.ListWallets = ListWallets;
const RemoveWallet = Db.RemoveWallet;
exports.RemoveWallet = RemoveWallet;
const SetBackend = Db.SetBackend;
